...
export async function persistAuthDecision(pool: Pool, request: AuthRequest, decision: AuthDecision): Promise<void> {
  ...
  await pool.query(
    `INSERT INTO auth_decision
       (auth_id, member_id, codes, decision, funding_source, co_payment, reimbursement_basis, length_of_stay, reasons, gate_results, rules_version, caveat, motivation_text)
     VALUES ($1, $2, $3::jsonb, $4, $5, $6::jsonb, $7, $8::jsonb, $9::jsonb, $10::jsonb, $11, $12, $13)`,
    [
      decision.authId,
      decision.memberId,
